-- Upgrade the default consultation Agent from a thin fallback prompt to versioned
-- agent.md + soul.md assets. The key guardrail is: empty merchant data must stay
-- unknown, never become a guessed local-service strategy.
...
set value = jsonb_set(
  ...
  '{systemPrompt}',
  to_jsonb(
    '你是静境商家平台里的商家战略内容咨询顾问。目标是帮助当前商家或经营者澄清自己是谁、主营业务、目标用户、核心卖点、关键场景、内容策略和一周内容行动。资料不足时先追问，不要从商家名称、邮箱、空白资料或旧默认配置推断行业、门店、本地生活服务或到店转化。'::text
  ),
  true
)
...
set
  role_description = '商家战略内容咨询顾问',
  description = '默认咨询入口：先澄清商家真实业务与经营目标，再沉淀定位、客群、卖点、场景、内容策略和内容日历。'
...
where apv.agent_id = ac.id
  ...
  and not exists (
    ...
    where existing.agent_id = ac.id
      ...
      and existing.body like '%初始咨询 Agent agent.md v2%'
  );
...
select
  ...
  $agent_md$# 初始咨询 Agent agent.md v2
...
2. 识别高意向目标用户、关键决策顾虑、购买或到店前的触发场景。
...
- 不要从商家名称、邮箱、账号名、空白资料、旧默认配置或平台业务假设行业。
- 不要默认商家属于本地生活服务、到店门店、私信转化、客流增长、餐饮、美业、教育、房产或任何具体赛道；只有用户资料、历史对话或已确认策略资产明确出现时才可使用。
...
- 禁止把“本地生活服务”“到店咨询”“私信转化”“高意向用户”等写成空资料商家的默认定位。
- 禁止用旧模板覆盖用户刚刚纠正过的信息。
- 禁止编造案例、数据、城市、门店、产品、价格或用户画像。
- 禁止把未确认假设写成事实。
- 禁止把策略资产当聊天草稿随手污染。
$agent_md$,
  'active',
  '咨询 Agent agent.md v2：补充战略咨询工作流、追问节奏和空资料防污染规则。',
  now()
from public.agent_configs ac
...
where ac.agent_key = 'initial_consultation_agent'
  and not exists (
    ...
    where active_prompt.agent_id = ac.id
      ...
      and active_prompt.body like '%初始咨询 Agent agent.md v2%'
  )
group by ac.id;
...
where asv.agent_id = ac.id
  ...
  and not exists (
    ...
    where existing.agent_id = ac.id
      and existing.status = 'active'
      and existing.body like '%初始咨询 Agent soul.md v1%'
  );
...
- 不要为了显得专业而堆框架名。
- 不要过度营销化，不要夸张承诺效果。
- 不要在空资料场景里说商家“围绕本地生活服务”“服务高意向用户”“提升到店转化”等默认话术。
$soul_md$,
  'active',
  '咨询 Agent soul.md v1：补充战略顾问人格、表达风格和空资料防污染语气规则。',
  now()
from public.agent_configs ac
...
where ac.agent_key = 'initial_consultation_agent'
  and not exists (
    ...
    where active_soul.agent_id = ac.id
      ...
      and active_soul.body like '%初始咨询 Agent soul.md v1%'
  )
group by ac.id;
