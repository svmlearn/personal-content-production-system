    ...
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex min-h-16 flex-col gap-3 border-b border-[#eadfd7] px-5 py-4 sm:flex-row sm:items-center sm:justify-between lg:px-8">
        <div>
          ...
          <p className="text-xs text-[#9b8d84]">邀请码、成员和一周内容分发对象</p>
        </div>
        <button
          type="button"
          onClick={() => void loadTeam()}
          className="inline-flex items-center gap-2 rounded-full border border-[#eadfd7] bg-[#fffaf7] px-4 py-2 text-xs text-[#6f625d] transition-colors hover:bg-[#fff0ef] hover:text-[#1f2328]"
        >
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-5 lg:px-6 lg:py-6">
        <div className="grid gap-4 xl:grid-cols-[0.9fr_1.1fr]">
          <section className="rounded-xl border border-[#eadfd7] bg-white p-5">
            <div className="flex items-center justify-between gap-3">
              <div>
                <div className="flex items-center gap-2 text-sm text-[#f2556b]">
                  <KeyRound className="size-4" aria-hidden="true" />
                  邀请码
                </div>
                <h2 className="mt-2 text-lg text-[#1f2328]">生成成员加入链接</h2>
              </div>
              {newestCode ? (
                <button
                  type="button"
                  onClick={() => void copyInviteLink(newestCode)}
                  className="inline-flex items-center gap-2 rounded-lg border border-[#f2556b]/25 bg-[#fff0ef] px-3 py-2 text-xs text-[#c46a00]"
                >
            ...
            <form onSubmit={(event) => void createInvitationCode(event)} className="mt-5 space-y-4">
              <Field label="自定义邀请码">
                <input
                  ...
                  placeholder="留空自动生成"
                  className="w-full rounded-lg border border-[#eadfd7] bg-[#fffaf7] px-3 py-3 text-sm text-[#1f2328] outline-none focus:border-[#f2556b]/50"
                />
              </Field>
              ...
              <button
                type="submit"
                disabled={creating}
                className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-[#f2556b] px-4 py-3 text-sm font-medium text-[#fffaf7] transition-opacity disabled:opacity-60"
              >
                {creating ? (
          ...
          <section className="rounded-xl border border-[#eadfd7] bg-white p-5">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2 text-sm text-[#1f2328]">
                <Users className="size-4 text-[#f2556b]" aria-hidden="true" />
                成员列表
              </div>
              <span className="rounded-lg border border-[#eadfd7] bg-[#fffaf7] px-2 py-1 text-xs text-[#8b7b72]">
            ...
            <div className="mt-4 divide-y divide-[#eadfd7]">
              {team?.members.length ? (
                ...
                <div className="rounded-lg border border-dashed border-[#eadfd7] px-4 py-8 text-center text-sm text-[#9b8d84]">
                  暂无成员
                </div>
              )}
            </div>
          ...
          <section className="rounded-xl border border-[#eadfd7] bg-white p-5 xl:col-span-2">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2 text-sm text-[#1f2328]">
                <ShieldCheck className="size-4 text-[#f2556b]" aria-hidden="true" />
                邀请码记录
              </div>
              <span className="rounded-lg border border-[#eadfd7] bg-[#fffaf7] px-2 py-1 text-xs text-[#8b7b72]">
            ...
            <div className="mt-4 overflow-x-auto">
              <table className="w-full min-w-[720px] text-left text-sm">
                <thead className="text-xs text-[#9b8d84]">
                ...
                <tbody className="divide-y divide-[#eadfd7]">
                  {team?.invitationCodes.length ? (
                    ...
                    <tr>
                      <td colSpan={6} className="py-8 text-center text-[#9b8d84]">
                        暂无邀请码
                      </td>
                    </tr>
                  )}
  ...
  return (
    <div className="flex items-center justify-between gap-4 py-4">
      <div className="flex min-w-0 items-center gap-3">
        <div className="flex size-10 shrink-0 items-center justify-center rounded-lg border border-[#eadfd7] bg-[#fffaf7]">
          <UserRound className="size-4 text-[#8b7b72]" aria-hidden="true" />
        </div>
        <div className="min-w-0">
          <p className="truncate text-sm text-[#1f2328]">{member.displayName || member.userId}</p>
          <p className="mt-1 truncate text-xs text-[#9b8d84]">{member.userId}</p>
        </div>
      </div>
      <span
        className={cn(
          ...
          member.role === "owner"
            ? "bg-[#fff0ef] text-[#c46a00]"
            : "bg-[#fffaf7] text-[#8b7b72]",
        )}
      >
...
function InvitationCodeRow({
  ...
  return (
    <tr className="text-[#5f514a]">
      <td className="py-3 pr-4 font-mono text-xs text-[#1f2328]">{invitationCode.code}</td>
      ...
      <td className="max-w-[220px] truncate py-3 pr-4 text-xs text-[#8b7b72]">
        {invitationCode.note || "-"}
      </td>
      <td className="py-3 text-right">
        <button
          type="button"
          onClick={onCopy}
          className="inline-flex items-center justify-center gap-2 rounded-lg border border-[#eadfd7] bg-[#fffaf7] px-3 py-2 text-xs text-[#6f625d] transition-colors hover:bg-[#fff0ef] hover:text-[#1f2328]"
        >
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="block">
      <p className="mb-2 text-xs text-[#9b8d84]">{label}</p>
      {children}
    </label>
  );
}
